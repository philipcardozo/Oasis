...
function dl(id,label,icon,source,layerType="symbol",color="#94a3b8",opts={}){
  return {id,label,icon,source,layerType,color,count:0,loaded:false,...opts};
}
const MARKETPLACE_ASSET_TYPES=[
...
const DATA_LAYER_BY_ID=Object.fromEntries(DATA_LAYER_PRESETS.flatMap(p=>p.layers.map(layer=>[layer.id,layer])));
const DATA_LAYER_OPEN=Object.fromEntries(DATA_LAYER_PRESETS.map((p,i)=>[p.id,i===0]));
